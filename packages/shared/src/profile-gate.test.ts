...
describe("profile gate", () => {
  it("excludes infrastructure titles only when the north star rejects them", () => {
    ...
    expect(gateListing({ title: "Ubuntu Software Engineer", locationRaw: "Remote" }, mina).reason).toBe("title_exclude:ubuntu");
    expect(gateListing({ title: "Senior Software Engineer - Data Platform", locationRaw: "Remote" }, mina).reason).toBe("title_exclude:data platform");
    expect(gateListing({ title: "Staff Software Engineer for Identity Management (Go/Golang)", locationRaw: "Remote" }, mina).pass).toBe(true);
