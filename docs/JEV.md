...
With Jev triage enabled, a new position's first JD and each meaningful JD change also queue a job-fit check. This runs independently of writing-model automation, including for positions already in the application or interview stages. It reads the latest JD and current profile and records its answer without changing scores, stages, drafts or approvals. Unchanged checks and cosmetic edits do not make another request. Closed, archived, rejected and quarantined positions are skipped, and an older queued revision is skipped when a newer one exists. Turning Jev triage off stops these checks too.

Automatic checks share cached answers with normal triage in the same mode. Manual previews have a separate cache so they do not hide automatic activity in history. The worker queue shows `jev_check` jobs; completed checks appear in the position's Jev history and timeline. Limits and missing credentials can skip a check without interrupting discovery or JD watching.
